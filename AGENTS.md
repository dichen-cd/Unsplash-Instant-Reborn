...
- **EXIF gate**: candidates with no displayable EXIF are dropped *before* ranking, so the bottom-right camera panel is never empty. `hasDisplayableExif()` deliberately mirrors the exact fields `displayPhoto()` prints (make, model, exposure_time, aperture, iso, focal_length) — keep the two in sync or the panel can render blank again. Unsplash strips EXIF from many uploads, so if a whole batch comes back bare the code falls back to the unfiltered set rather than showing nothing; that path logs a `console.debug`. Filtering shrinks the effective batch, which just makes the low-water refill fire more often.
- **Quality scoring**: `scoreCandidate()` ranks on resolution vs. the target width (quadratic penalty below it), aspect-ratio fit to the viewport, and EXIF completeness. Because the EXIF gate runs first, the EXIF term now only separates partial from complete camera data. This replaced an older loop that spent up to 3 API calls retrying for complete EXIF.
